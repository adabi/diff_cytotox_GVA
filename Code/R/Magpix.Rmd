...
df_test_scaled <-
  df_test %>% 
  mutate_at(vars(-Group, -Analyte, -mean_ctrl, -row), funs(((. - mean_ctrl)/sd(., na.rm = TRUE) + 2))) %>% 
  select(-mean_ctrl) %>% 
  pivot_longer(cols = c('0', '20', 'LPS'), names_to = 'Treatment', values_to = 'Value') 

df_sep <- 
  df %>% 
  separate("Group", into=c("Group", "Treatment"), sep=" ")

# Here we normalize so that the neam of the negative control is 2 (2 was chosen to avoid negative numbers for some analytes)
df_splt_sep <-
  df_sep %>% 
  split(.$Group)
...
df_scaled <-
  df_scaled %>% 
  filter(Treatment != "20") %>% 
  mutate(Treatment = replace(Treatment, Treatment=='LPS', '20'))
