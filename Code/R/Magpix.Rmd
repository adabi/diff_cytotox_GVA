...
df_stat_summary <- 
  ...
  write.csv(subsampled_df, "subsampled_df.csv")
...
df <- read.csv(file="Full Raw Data Alternative.csv", check.names = FALSE)
...
ggsave('./Graphs/MANCOVAByGroup.png', device ='png')
...
ggsave('./Graphs/MANCOVAByTreatment.png', device ='png')
```
...
df_scaled_sig <-
  ...
  mutate(p_val = wilcox.test(unlist(treatment_0), unlist(treatment_20))$p.value) %>% 
...
ggplot(df_test, height= 30) +
  geom_bar(aes(x = Analyte, y=mean), stat='identity', fill = 'deepskyblue3') +
  geom_errorbar(aes(x=Analyte, ymin = mean-stderr, ymax= mean + stderr), width= 0.2) +
  geom_text(aes(x=Analyte, y = mean+stderr + 0.3, label=aestrik), size=5) +
...
ggsave("./Graphs/everthang.png", device='png', height = 6.5, width = 6.5)
